...
const HEADERS = {
  HIRING:['ID','Vendor Name','Hiring Date','Full Name','Contact Number','Alt Contact Number',
    'Work Experience','Languages','Interview Date','Interview By','Interview Status',
    'Rejected Reason','Selected Process','Training Date','Training By','Training Status',
    'Dropout Reason','Date of Joining','Certification By','Certification Status',
    'Not Certified Reason','Certified Date','Final Status','Added By','Added On','Updated By','Updated On'],
  REQUIREMENT:['ID','Title','Language','Vendor','Count Required','Count Filled','Status','Created By','Created On'],
...
// KEY FIX: convert all Sheet cell values to safe primitives (fixes data-not-showing bug)
function _s(v) {
...
// Returns user + DV data in ONE call (halves startup latency)
function initApp() {
  try {
    ...
    try { const p=People.People.get('people/me',{personFields:'names'}); if(p.names&&p.names[0]) name=p.names[0].displayName||name; } catch(e){}
    ...
    if (!acc.found) {
      ...
      if (rows.length===0) {
        ...
        return {ok:true,email,name,role:'Admin',company:'Porter',dv:_getDVData(ss)};
      }
    ...
    return {ok:true,email,name,role:acc.role,company:acc.company,dv:_getDVData(ss)};
...
function getHiringData(ctx) {
  try {
    ...
    if (ctx&&ctx.role==='Supervisor') rows = rows.filter(r=>_s(r[1])===ctx.company);
    // ★ Convert every cell to a safe string — this is the fix for data-not-showing
...
function saveAccess(rec,ctx){
  try{
    const ss=SpreadsheetApp.openById(CONFIG.SHEET_ID);const sh=ss.getSheetByName(SN.ACCESS);
    const d=sh.getDataRange().getValues();const email=Session.getActiveUser().getEmail();
    let ri=-1;for(let i=1;i<d.length;i++)if(d[i][0]&&_s(d[i][0]).toLowerCase()===rec.email.toLowerCase()){ri=i+1;break;}
    if(ri>0){sh.getRange(ri,1,1,6).setValues([[rec.email,rec.name,rec.role,rec.company,email,_s(d[ri-1][5])]]);_writeLog(ss,email,ctx.name||'','UPDATE_ACCESS','Updated '+rec.email,'');}
    else{sh.appendRow([rec.email,rec.name,rec.role,rec.company,email,new Date().toISOString()]);_writeLog(ss,email,ctx.name||'','ADD_ACCESS','Added '+rec.email,'');}
    return{ok:true};
  }catch(e){return{ok:false,msg:e.message};}
}
function removeAccess(targetEmail,ctx){
  try{
    const ss=SpreadsheetApp.openById(CONFIG.SHEET_ID);const sh=ss.getSheetByName(SN.ACCESS);
    const d=sh.getDataRange().getValues();
    for(let i=1;i<d.length;i++)if(d[i][0]&&_s(d[i][0]).toLowerCase()===targetEmail.toLowerCase()){sh.deleteRow(i+1);_writeLog(ss,Session.getActiveUser().getEmail(),ctx.name||'','REMOVE_ACCESS','Removed '+targetEmail,'');return{ok:true};}
    return{ok:false,msg:'Not found'};
  }catch(e){return{ok:false,msg:e.message};}
}
...
function getUserBaseData(){
  try{
    const d=SpreadsheetApp.openById(CONFIG.SHEET_ID).getSheetByName(SN.USERBASE).getDataRange().getValues();
    if(d.length<=1)return{ok:true,data:[],summary:{total:0,admin:0,supervisor:0}};
    const logs=d.slice(1).filter(r=>r[0]).map(r=>({email:_s(r[0]),name:_s(r[1]),role:_s(r[2]),company:_s(r[3]),date:_s(r[4]),time:_s(r[5])}));
    const unique=[...new Set(logs.map(l=>l.email))];const roleOf={};
    logs.forEach(l=>{if(!roleOf[l.email])roleOf[l.email]=l.role;});
    return{ok:true,data:[...logs].reverse(),summary:{total:unique.length,admin:unique.filter(e=>roleOf[e]==='Admin').length,supervisor:unique.filter(e=>roleOf[e]==='Supervisor').length}};
  }catch(e){return{ok:false,msg:e.message};}
}
